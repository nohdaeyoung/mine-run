...
const GRADE_STYLES: Record<ComboGrade, { text: string; size: string; color: string }> = {
  TAP: { text: '', size: '', color: '' },
  NICE: { text: 'Nice~', size: 'text-xl', color: 'text-[#5b8a4e]' },
  GREAT: { text: 'Wonderful!', size: 'text-3xl', color: 'text-[#4a7cbc]' },
  AMAZING: { text: 'Amazing!!', size: 'text-4xl', color: 'text-[#7b6d9a]' },
  FEARLESS: { text: 'Incredible!!!', size: 'text-5xl', color: 'text-[#b8901a]' },
};
  ...
  return (
    <div
      ...
      className={`fixed top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2
        pointer-events-none z-50 animate-bounce
        ${style.size} ${style.color} font-extrabold tracking-wide`}
      style={{ textShadow: '0 2px 8px rgba(255,255,255,0.8)' }}
    >
      <div>{style.text}</div>
      <div className="text-center text-xs opacity-60 text-[#8b7d6b]">
