...
const GRADE_STYLES: Record<ComboGrade, { text: string; size: string; color: string }> = {
  TAP: { text: '', size: '', color: '' },
  NICE: { text: 'Nice~', size: 'text-xl', color: 'text-green-400' },
  GREAT: { text: 'Great!', size: 'text-3xl', color: 'text-blue-400' },
  AMAZING: { text: 'Amazing!!', size: 'text-4xl', color: 'text-purple-400' },
  FEARLESS: { text: 'FEARLESS!!!', size: 'text-5xl', color: 'text-orange-400' },
};
  ...
  return (
    <div
      ...
      className={`
        ...
        drop-shadow-[0_2px_8px_rgba(244,114,182,0.5)]
      `}
    >
      <div>{style.text}</div>
      <div className="text-center text-sm font-mono opacity-80 text-pink-500">
